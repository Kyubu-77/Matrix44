...
// Stripe movement
const float STIPE_MOVE_PX_PER_MS					= 0.10f / 1.0f;  
...
// > 1  letter moves faster (not tested)
...
// just the existing letter should fall down one font hight
// If there is space for more than one new letter several letters may be inserted
const float   STATIC_LETTERS_PROBAPILITY				= 0.1f;
...
// Letter fading out 
const float LETTER_ALPHA_DEGREASE_AFTER_MS = 1500.0f;
const float LETTER_ALPHA_DEGREASE_PER_MS = 3.0f;
const float LETTER_ALPHA_DEGREASE_PER_MS_VARIATON = 0.3f;

// Letters
const uint16_t MAX_LETTERS			= 300;
const uint8_t  LETTER_ALPHA_BITS	= 5; // 32 fading steps
const uint8_t  ALPHA_COUNT			= (0x1 << LETTER_ALPHA_BITS);
const float    ALPHA_FACTOR			= (ALPHA_COUNT-1) / 255.0f;
